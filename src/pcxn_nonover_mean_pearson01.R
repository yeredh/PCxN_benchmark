...
# RUN ON ODYSSEY (pcxn_nonover_mean_pearson01.sh) [1-863]
...
ProcessElement = function(ic){
  ...
  summaryAu = GetSummary(dat=exprs_rnk,gs=gsAu,sum_fun=colMeans)
  summaryBu = GetSummary(dat=exprs_rnk,gs=gsBu,sum_fun=colMeans)
  ...
  tmp = c(tmp,GetCorEstimates(x=summaryAu,y=summaryBu,METHOD = "pearson"))
...
# save results
saveRDS(res,paste0(pcxn_dir,"output/Benchmark/NoOverlap/Mean/Pearson/GSE/",gse,"_ribosome_nonover.RDS"))
rm(list=ls())
