# Aggregate the experiment-level estimates using the 
# correlation shrinkage estimator
...
for(io in 1:2){
  for(jo in 1:2){
    for(ko in 1:2){
      ...
      # Ribosome Gene Sets
      source(paste0(pcxn_dir,"src/pcxn_benchmark_res02a.R"))
      # Random Gene Sets
      source(paste0(pcxn_dir,"src/pcxn_benchmark_res02b.R"))
    }
  }
}
cat("DONE \n\n")
